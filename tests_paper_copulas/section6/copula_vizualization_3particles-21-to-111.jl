...
B = [-5.0, -5.0, -5.0]
C = [-3.0, -1.0, 0. ]
A = [5.0,  5.0, 5.0] #spatial coef for the potential
...
for (i, a) in enumerate(A)
   ...
   vext(x) = -α * (1 ./ sqrt.((x-b) .^ 2 .+ 1) .+ 1. ./ sqrt.((x-c).^ 2 .+ 1) .+ 1.0 ./ sqrt.((x-a).^ 2 .+ 1))
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation21-111/external_pot_$i.pdf")

   display(P)
   sleep(0.5)

   P = plot(xfine, ρsp.(xfine), label="", xlims=(-L, L), linewidth=4, tickfontsize=16)
   savefig("plots_test_cases_paper_copulas/section5/dissociation21-111/density_$i.pdf")
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation21-111/pair_density$i.pdf")

   display(P)
   sleep(0.5)

   P = contour(xx, xx, ρ2MF, c=:viridis, aspect_ratio=:equal, colorbar=:true, fill=:true, tickfontsize=16, colorbar_tickfontsize=16)
   savefig("plots_test_cases_paper_copulas/section5/dissociation21-111/mean_field$i.pdf")
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation21-111/copula_$i.pdf")
