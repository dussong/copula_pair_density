...
A = [2., 3., 5.] #spatial coef for the potential
α_lap = 1.0
B = [-2., -3.,-5.]
...
for (i, a) in enumerate(A)
   ...
   tick()
   _, Ψ = WaveFunction(ne, ham, "FCI_sparse2"; maxiter=500, kdim=50)
   tock()
   # _, Ψ = WaveFunction(ne, ham, "selected_CI_sparse"; max_iter=15000, k=500, num=500, a0=[[b - 1, b + 1, a - 1, a + 1]])
   # WaveFunction(ne, ham, "selected_CI_sparse"; max_iter=3000, k=500, num=500)
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation22/external_pot_$i.pdf")

   # display(P)
   # sleep(0.5)

   P = plot(xfine, ρsp.(xfine), label="", xlims=(-L, L), linewidth=4, tickfontsize=16)
   savefig("plots_test_cases_paper_copulas/section5/dissociation22/density_$i.pdf")
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation22/pair_density$i.pdf")

   # display(P)
   # sleep(0.5)

   P = contour(xx, xx, ρ2MF, c=:viridis, aspect_ratio=:equal, colorbar=:true, fill=:true, tickfontsize=16, colorbar_tickfontsize=16)
   savefig("plots_test_cases_paper_copulas/section5/dissociation22/mean_field$i.pdf")
   ...
   savefig("plots_test_cases_paper_copulas/section5/dissociation22/copula_$i.pdf")
...
# vext(x) = -α * (1.0 ./ sqrt.((x - a - 1.0) .^ 2 .+ 1)
#                 .+
#                 1.0 ./ sqrt.((x - a + 1.0) .^ 2 .+ 1)
#                 .+
#                 1.0 ./ sqrt.((x - b - 1.0) .^ 2 .+ 1)
#                 .+
#                 1.0 ./ sqrt.((x - b + 1.0) .^ 2 .+ 1))
# #double well taken from Wagner, L.O., Stoudenmire, E.M., Burke, K., White, S.R.: Reference electronic structure calculations in one dimension. Phys. Chem. Chem. Phys. 14, 8581–8590 (2012). https://doi.org/10.1039/c2cp24118h
# vee(x) = 1.0 ./ sqrt.(x .^ 2 .+ 1)
...
# # Solve the eigenvalue problem
# # _, Ψ = WaveFunction(ne, ham, "FCI_full"; maxiter=500, kdim=3)
# _, Ψ = WaveFunction(ne, ham, "selected_CI_sparse"; max_iter=300, k=500, num=500)
